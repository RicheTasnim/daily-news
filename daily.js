const newsCategories = async () => {
  ...
  const displayNews = cardPosts => {
    const displayPost = document.getElementById('displayNewsPost');
    ...
    <div class="row g-0">
      ...
      <div class="col-md-8">
        <div class="card-body ">
          ...
          <div class="d-flex">
          <div>
          <img class="author-img rounded-circle" src="${cardPost.author.img ? cardPost.author.img : 'No'}" alt="">
          </div>
          <div class="ms-3">
          <p class="fw-bold ">${cardPost.author.name ? cardPost.author.name : 'No name found'}</p>
          <p>${cardPost.author.published_date ? cardPost.author.published_date : 'No date infoormation'}</p>
          </div>
          </div>
  
          <div>
          <p class="fw-semibold"><i class="fa-regular fa-eye"></i> ${cardPost.total_view ? cardPost.total_view : 'OOps!!! No views'}</p>
          </div>
